...
public class AddMovie  extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Connection connection = DriverManager.getConnection("jdbc:mysql://52.40.189.210:3306/moviedb","root", "cs122b");
		...
		//StoredProcedures newStatements = new StoredProcedures("root","cs122b","jdbc:mysql://52.40.189.210:3306/moviedb"); 
		...
		//did not insert movie, forward to same page
		if(firstWord.equals("movie"))
		{
			
			/*
			String sessionURL = response.encodeRedirectURL(request.getContextPath() + "/_dashboard/addmovie");
			response.sendRedirect(sessionURL);
			*/
			RequestDispatcher dispatcher = request.getRequestDispatcher("/_dashboard/addmovie");
			dispatcher.forward(request, response);
			connection.close();
		}
		
		//inserted movie, redirect to addinfo page
		else
		{	
			session.setAttribute("log", log);

			session.setAttribute("insert-check", "passed");
			
		String sessionURL = response.encodeRedirectURL(request.getContextPath() + "/_dashboard/addinfo");
		response.sendRedirect(sessionURL);
		connection.close();
		}

...
<%
String title = (String)session.getAttribute("form-title");
String year = (String)session.getAttribute("form-year");
String director = (String)session.getAttribute("form-director");
if(title == null) {
	title = "";
}
if(year == null) {
	...
	<div class="row">
		<div class="col-sm-8 col-sm-offset-2">
			...
			<div class="row">
				...
				<div class="col-sm-3"><a class="btn btn-info center-block" href="<%=request.getContextPath()%>/_dashboard/addinfo">Add info</a></div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-sm-4 col-sm-offset-4">	
			...
			<div class="row">
				...
				%>
				<div class="alert alert-warning" role="alert"><%=(String)request.getAttribute("log")%></div>
				...
				%>
						...
						<% 
				if(session.getAttribute("insert-check") == "passed") {
				%>
				<div class="alert alert-warning" role="alert"><%=(String)session.getAttribute("log")%></div>
				
				<% 
				session.setAttribute("insert-check", "reset");
				}
				%>
			</div>
			<form action="<%=request.getContextPath()%>/_dashboard/addInfoServlet" method="post"> 
				<div class="row">
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4">Movie Title</div>
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4"><input type="text" name="movieTitle" value="<%= title %>" /></div>
				</div>
				<div class="row">
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4">Year</div>
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4"><input type="text" name="year" placeholder="YYYY" value="<%= year %>" pattern="[0-9]{4}"/></div>
				</div>	
				<div class="row">
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4">Director</div>
					<div class="col-sm-6 col-sm-offset-0 col-xs-4 col-xs-offset-4"><input type="text" name="director" value="<%= director %>" /></div>
				</div>
				<div class="row">

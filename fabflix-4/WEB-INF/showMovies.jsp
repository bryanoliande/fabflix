	...
	<div class="row text-center"><h1>Search Results</h1></div>
		...
		%>
			...
			<tr>
				...
				<td><%=movie.getId() %></td>
				<td><a href="<%=request.getContextPath()%>/movie/?id=<%=movie.getId()%>" onmouseover="ajax_showTooltip(window.event, '<%=request.getContextPath()%>/ajaxMovie?id=<%=movie.getId()%>', this);return false"> <%=movie.getTitle() %></a></td>
				<td><%=movie.getYear()%></td>
